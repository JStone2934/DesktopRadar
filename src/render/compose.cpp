...
bool composeAnimFillPast(LGFX* lcd, float lat, float lon, int zoom) {
  if (!lcd || !zoomCanCompose(zoom) || !frameCacheHas(zoom)) {
    return false;
  }

  RainviewerFrame past[ANIM_MAX_FRAMES];
  int pastCount = 0;
  if (!rainviewerFetchPast(past, ANIM_MAX_FRAMES, &pastCount, ANIM_WINDOW_HOURS) ||
      pastCount < 2) {
    Serial.printf("anim fill past fail count=%d\n", pastCount);
    return false;
  }

  composeClearAbort();
  Serial.printf("anim fill past z%d frames=%d\n", zoom, pastCount);
  reportComposeProgress(zoom, 0.02f);

  // 只保证至少 2 帧可播的空间，避免为凑满 10 帧拆掉邻档静帧
  if (!frameCacheAnimEnsureSpace(2UL * FRAME_RGB565_BYTES + (48UL * 1024UL),
                                 zoom)) {
    Serial.println("anim fill ensureSpace fail");
    return false;
  }
  if (!frameCacheAnimBeginFill(zoom)) {
    return false;
  }
  ...
  if (scale > 1) {
    ...
    rty0 = (int)floor(vp.origin_py / ((double)TILE_SIZE * scale));
    rtx1 =
        (int)floor((vp.origin_px + LCD_WIDTH - 1) / ((double)TILE_SIZE * scale));
    rty1 = (int)floor((vp.origin_py + LCD_HEIGHT - 1) /
                      ((double)TILE_SIZE * scale));
  }

  const int baseTiles = (vp.tx1 - vp.tx0 + 1) * (vp.ty1 - vp.ty0 + 1);
  const int radarTiles = (rtx1 - rtx0 + 1) * (rty1 - rty0 + 1);

  frameCacheScrubOrphansExcept(zoom);

  auto fetchBasemapOnce = [&](int tx, int ty) -> bool {
    return fetchTileToFs(zoom, false, tx, ty, basemapTileUrl(zoom, tx, ty),
                         AMAP_REFERER, 800, "anim-base");
  };
  ...
  if (baseOk < baseTiles) {
    Serial.printf("anim fill basemap dl incomplete %d/%d\n", baseOk, baseTiles);
    return false;
  }
  reportComposeProgress(zoom, 0.15f);
  ...
  if (baseDecoded < baseTiles) {
    Serial.printf("anim fill basemap decode incomplete %d/%d\n", baseDecoded,
                  baseTiles);
    return false;
  }
  reportComposeProgress(zoom, 0.25f);

  uint16_t* frame = (uint16_t*)malloc(FRAME_RGB565_BYTES);
  if (!frame) {
    Serial.println("anim fill malloc fail");
    return false;
  }
  const uint16_t bg = backdropColor(lcd);
  for (size_t i = 0; i < (size_t)LCD_WIDTH * LCD_HEIGHT; ++i) {
  ...
  auto stampOne = [&](bool isRadar, int tx, int ty, int sc, bool withAlpha)
      -> bool {
    snprintf(rawPath, sizeof(rawPath), "/frames/z%02d/%c_%d_%d.raw", zoom,
             isRadar ? 'r' : 'b', tx, ty);
    if (!LittleFS.exists(rawPath)) {
    ...
    const int oy =
        (int)lround((double)ty * TILE_SIZE * sc - vp.origin_py);
    const bool ok =
        frameCacheStampRawToBuffer(frame, rawPath, ap, ox, oy, sc, withAlpha,
                                   nullptr);
    LittleFS.remove(rawPath);
    if (ap) {
      LittleFS.remove(ap);
  ...
  if (baseStamped < baseTiles) {
    Serial.printf("anim fill basemap stamp incomplete %d/%d\n", baseStamped,
                  baseTiles);
    free(frame);
    return false;
  }
  ...
  frameCacheDrawCrosshairBuf(frame, lcd->color565(255, 255, 255));
  if (!frameCacheAnimWriteBase(zoom, frame)) {
    Serial.println("anim fill writeBase fail");
    free(frame);
    return false;
  }
  reportComposeProgress(zoom, 0.30f);

  uint32_t staticTime = 0;
  frameCacheReadRadarTime(zoom, &staticTime);
  ...
  for (int i = 0; i < pastCount; ++i) {
    ...
    if (composeAbortRequested()) {
      free(frame);
      frameCacheAnimRemoveBase(zoom);
      frameCacheAnimClear(zoom);
      return false;
    }

    const bool useStatic =
        (i == pastCount - 1) && staticTime != 0 && past[i].time == staticTime;
    if (useStatic) {
      if (!frameCacheLoadRgb565(zoom, frame)) {
        Serial.println("anim fill load static fail");
        free(frame);
        frameCacheAnimRemoveBase(zoom);
        frameCacheAnimClear(zoom);
        return false;
      ...
      if (!frameCacheAnimReadBase(zoom, frame)) {
        Serial.println("anim fill readBase fail");
        free(frame);
        frameCacheAnimRemoveBase(zoom);
        frameCacheAnimClear(zoom);
        return false;
      ...
      for (int ty = rty0; ty <= rty1; ++ty) {
        for (int tx = rtx0; tx <= rtx1; ++tx) {
          ...
          if (fetchTileToFs(zoom, true, tx, ty,
                            rainviewerTileUrl(past[i], overlayZoom, tx, ty),
                            nullptr, 200, "anim-radar")) {
            ++radarOk;
          }
        }
      }
      if (radarOk == 0) {
        Serial.printf("anim fill radar miss i=%d t=%lu — skip\n", i,
                      (unsigned long)past[i].time);
        continue;
      }
      ...
      for (int ty = rty0; ty <= rty1; ++ty) {
        for (int tx = rtx0; tx <= rtx1; ++tx) {
          ...
          decodeOne(true, tx, ty, true);
        }
      }
      for (int ty = rty0; ty <= rty1; ++ty) {
        for (int tx = rtx0; tx <= rtx1; ++tx) {
          stampOne(true, tx, ty, scale, true);
        }
      }
      frameCacheDrawOverlayBuf(frame, past[i].time);
    }

    if (!frameCacheAnimWriteFrame(zoom, written, frame)) {
      Serial.printf("anim fill writeFrame %d fail (keep %d)\n", written,
                    written);
      break;
    }
    times[written] = past[i].time;
    ++written;
    reportComposeProgress(
        zoom, 0.30f + 0.65f * ((float)(i + 1) / (float)pastCount));
    Serial.printf("anim fill wrote i=%d/%d t=%lu\n", written, pastCount,
                  (unsigned long)past[i].time);
    if (written >= ANIM_MAX_FRAMES) {
      break;
    }
  }
  ...
  if (written < 2) {
    Serial.printf("anim fill too few written=%d\n", written);
    frameCacheAnimClear(zoom);
    return false;
  }

  if (!frameCacheAnimCommitMeta(zoom, written, times)) {
    Serial.println("anim fill commitMeta fail");
    frameCacheAnimClear(zoom);
    return false;
  }

  reportComposeProgress(zoom, 1.0f);
  Serial.printf("anim fill done z%d count=%d\n", zoom, written);
  (void)radarTiles;
  return true;
}
