...
| `adsb_sweep` | ADSB 飞机雷达 + 老式扫描余晖 | 实时 |
...
| `adsb_sweep` | 老式 PPI 雷达：慢速旋转的绿色渐变余晖；真实扫描规则（见下） |

圆屏顶部显示量程与飞机数量；最近若干架标注呼号与高度（FL）。飞机按高度分色（地面/低空/中空/高空）。

**平滑移动（插值）**：`adsb_radar` / `adsb_map` / `adsb_outline` 三种图层：ADSB 数据约每 8 秒更新一次，圆屏以约 8fps 重绘，并按航向（`track`）与地速（`gs`）外推到当前时刻，实现丝滑连续移动。

**真实扫描（`adsb_sweep` 例外）**：后台持续拉取最新 ADSB 数据，但屏幕与实时位置解耦——绿色余晖与 `_sweep_wedge_contains` 使用同一指南针坐标；**最新方位**落入当前余晖扇区（前沿顺时针，拖影在后 150°）时才更新显示，扇区外冻结；未进入余晖的不显示。

**1602 LCD**：在飞机通道，仅当雷达范围内**有飞机**时点亮背光 5 秒，显示最靠近本机的航班代码与距离（如 `Nearest` / `CCA742 12km`）；无飞机（No Traffic）时不亮屏。用户操作（切图层、调量程等）仍会照常点亮 LCD。
